...
impl Maze {
    ...
    fn navigate_maze(&self, start_points: &Vec<(usize, usize)>) -> i32 {
        let with_distances: Vec<((usize, usize), i32)> =
            start_points.iter().map(|x| (x.to_owned(), 0)).collect();
...
mod tests {
    ...
    fn it_works() {
        ...
        let mut start = maze.find_elem('S');
        assert_eq!(maze.navigate_maze(&start), 31);
        start.extend(maze.find_elem('a'));
        assert_eq!(maze.navigate_maze(&start), 29)
    }
    ...
    fn bench_loading(b: &mut Bencher) {
        let maze = Maze::new("test.txt");
        let mut start = maze.find_elem('S');
        b.iter(|| maze.navigate_maze(&start));
    }
}

    ...
    match cap {
        Some(cap) => {
            Some((
                ...
                cap.get(3).unwrap().as_str().parse::<i32>().unwrap()..cap.get(4).unwrap().as_str().parse::<i32>().unwrap(),
            ))
        },
        None => None,
    }
...
fn range_encaps_another(x: &Option<(Range<i32>, Range<i32>)>) -> bool {
    ...
    r1.end >= r2.end && r1.start <= r2.start ||
    r2.end >= r1.end && r2.start <= r1.start
}
...
fn main() {
    ...
    println!("{}", contents.split("\n").map(|line|
        find_ranges(line)
    ).filter(|x| x.is_some()).filter(|x|
        range_encaps_another(x)
    ).count());
    println!("{}", contents.split("\n").map(|line|
        find_ranges(line)
    ).filter(|x| x.is_some()).filter(|x|
...
mod tests {
  ...
  fn overlaps() {
    ...
    let r6 = Some((2..4,4..8));
    assert_eq!(
        ranges_overlap(&r1), true
    );
    assert_eq!(
        ranges_overlap(&r2), false
    );
    assert_eq!(
        ranges_overlap(&r3), true
    );
    assert_eq!(
        ranges_overlap(&r4), true
    );
    assert_eq!(
        ranges_overlap(&r5), true
    );
    assert_eq!(
        ranges_overlap(&r6), true
    )
  }
}

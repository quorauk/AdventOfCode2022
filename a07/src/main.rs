  ...
  fn ls(&mut self, lines: &str) {
    ...
    for line in lines.split('\n') {
      if re_dir.is_match(line) {
        ...
        self.push(FileSystemEntry::Dir(Directory { name: captures.name("name").unwrap().as_str().to_string()}))
      } else if re_file.is_match(line) {
        let captures = re_file.captures(line).unwrap();
        self.push(FileSystemEntry::File(File { name: captures.name("name").unwrap().as_str().to_string(), size: captures.name("size").unwrap().as_str().parse().unwrap()}))
      }
    }
  }
...
fn build_tree(contents: String) -> Tree<FileSystemEntry> {
    ...
    let mut tree = Tree::new(FileSystemEntry::Dir(Directory { name: "/".to_string()}));
...
fn main() {
    ...
    let sum : i32 = tree.dir_sizes().into_values().into_iter().filter(|y| *y < 100000).map(|y| y.clone()).collect::<Vec<i32>>().iter().sum();
    println!("part 1: {:?}", sum);
    ...
    let smallest_viable = tree.dir_sizes().into_values().into_iter().filter(|x| x > &to_delete).min();
